...
%inline expr_attrs:
  ...
  | LET OPEN override_flag ext_attributes module_expr IN seq_expr
      { let open_loc = make_loc ($startpos($2), $endpos($5)) in
        ...
        Pexp_open(od, $7), $4 }
  | FUNCTION ext_attributes match_cases END
      { Pexp_function $3, $2 }
  | FUN ext_attributes labeled_simple_pattern fun_def END
      { let (l,o,p) = $3 in
        Pexp_fun(l, o, p, $4), $2 }
  | FUN ext_attributes LPAREN TYPE lident_list RPAREN fun_def END
      { (mk_newtypes ~loc:$sloc $5 $7).pexp_desc, $2 }
  | MATCH ext_attributes seq_expr WITH match_cases END
      { Pexp_match($3, $5), $2 }
  | TRY ext_attributes seq_expr WITH match_cases END
      { Pexp_try($3, $5), $2 }
  | TRY ext_attributes seq_expr WITH error END
      { syntax_error() }
