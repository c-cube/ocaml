...
/* Representation of channel status and direction:
   ...
   Open channels have fd >= 0
     Open input channels have max != NULL
     Open output channels have max == NULL

   Channels are closed via caml_finalize_channel (GC finalizer), which sets fd = -1.
...
/* End of internal C buffered I/O.
   All OCaml-facing channel primitives have been removed — the channel
   system is now implemented in pure OCaml (stdlib/stdlib.ml).
   The functions above are only used by other C code: debugger.c,
   extern.c, intern.c, startup_byt.c, backtrace_byt.c. */
